...
export async function sendEmail(to: string, subject: string, body: string) {
  ...
  console.log('[EMAIL]', { to, subject, body: body.slice(0, 80), smtpUrlConfigured: !!smtpUrl })
  return { success: true }
}

export async function sendSMS(to: string, body: string) {
  const smsKey = process.env.SMS_API_KEY || ''
  console.log('[SMS]', { to, body: body.slice(0, 80), smsConfigured: !!smsKey })
  return { success: true }
}

export async function sendWhatsApp(to: string, body: string) {
  const waKey = process.env.WHATSAPP_API_KEY || ''
  console.log('[WHATSAPP]', { to, body: body.slice(0, 80), waConfigured: !!waKey })
  return { success: true }
}
