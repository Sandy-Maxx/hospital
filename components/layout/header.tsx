...
import { Bell, Search, User, LogOut, Settings, UserCircle2 } from 'lucide-react'
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative" ref={notifRef}>
            <Button variant="ghost" size="icon" className="relative" onClick={() => { setShowNotifs((s) => !s); if (!showNotifs) loadNotifs() }} aria-label="Notifications">
              <Bell className="w-5 h-5" />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-red-500 text-white rounded-full text-[10px] flex items-center justify-center">{unreadCount}</span>
              )}
            </Button>
            {showNotifs && (
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    <div key={n.id} className={`px-4 py-3 border-b hover:bg-gray-50 ${!n.isRead ? 'bg-blue-50' : ''}`}
                         ...
                         role="button" title="Mark as read">
