...
export default function CampaignViewModal({
  ...
  const audienceSummary = useMemo(() => {
    ...
    if (Array.isArray(a.conditions) && a.conditions.length)
      parts.push(`conditions:${a.conditions.join("|")}`);
    return parts.join(" • ");
  }, [campaign]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-sm text-gray-700 space-y-1">
                ...
                <div>
                  <span className="font-medium">Channels:</span> {channels}
                </div>
                <div>
              ...
              {campaign.result ? (
                <div className="text-sm text-gray-700 space-y-1">
                  <div>
                    ...
                    {campaign.result.sent || 0}
                  </div>
                  <div>
                    <span className="font-medium">Failed:</span>{" "}
                    {campaign.result.failed || 0}
                  </div>
                  <div>
                    <span className="font-medium">Total audience:</span>{" "}
                    {campaign.result.total || 0}
                  </div>
                </div>
              ) : (
        ...
        <Card>
          ...
          <CardContent>
            <div
              className="border rounded p-4 bg-white text-gray-900"
