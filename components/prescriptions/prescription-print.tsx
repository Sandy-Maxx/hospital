...
export default function PrescriptionPrint({ open, onClose, id }: Props) {
  ...
  const prescNo = data
    ? formatPrescriptionNumber({ id: data.id, createdAt: data.createdAt })
    : "";
          ...
          <div ref={printRef}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#374151" }}>
                    Prescription No: {prescNo}
                  </div>
                </div>
              </div>
              {qrUrl ? (
                <img alt="QR" src={qrUrl} width={100} height={100} />
              ) : (
                <div
                  style={{
                    width: 100,
            ...
            <div className="section" style={{ fontSize: 12 }}>
              <div>
                ...
                {data.patient.lastName} • <strong>Age:</strong>{" "}
                {data.patient.age || "-"} • <strong>Gender:</strong>{" "}
                {data.patient.gender || "-"}
              </div>
              <div>
            ...
            <div className="section">
              ...
              {meds.labTests && meds.labTests.length > 0 && (
                <div className="section" style={{ fontSize: 12 }}>
                  ...
                  <ul>
                    {meds.labTests.map((t: any, i: number) => (
                      <li key={i}>
                        {t.name} {t.instructions ? `- ${t.instructions}` : ""}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {meds.therapies && meds.therapies.length > 0 && (
                <div className="section" style={{ fontSize: 12 }}>
                  ...
                  <ul>
                    {meds.therapies.map((t: any, i: number) => (
                      <li key={i}>
                        {t.name} {t.frequency ? `• ${t.frequency}` : ""}{" "}
            ...
            <div className="footer">
              <div style={{ fontSize: 12, color: "#6b7280" }}>
                This prescription is digitally generated. For verification, scan
                the QR code.
              </div>
              <div className="sign">
                <div style={{ marginBottom: 6 }}>
                  <img
                    src={`/uploads/users/${data.doctor?.id}/signature.png`}
                    alt="Signature"
                    style={{ maxHeight: 48, maxWidth: 220 }}
                    onError={(e: any) => {
