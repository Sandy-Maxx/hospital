...
export default function TokenPrint({ appointment, hospitalSettings, onClose }: TokenPrintProps) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex space-x-2 justify-center mb-4">
          ...
          <Button onClick={handleScreenshotDownload} variant="outline" className="flex items-center">
        ...
        {/* Token Preview */}
        <div ref={printRef} className="token-container max-w-md mx-auto border-2 border-gray-300 rounded-lg p-6 bg-white shadow-lg">
          {/* Header */}
          <div className="header text-center border-b-2 border-gray-200 pb-4 mb-4">
            {hospitalSettings.logo && (
              <img src={hospitalSettings.logo} alt="Hospital Logo" className="mx-auto mb-2" style={{ maxHeight: 64, width: 'auto', objectFit: 'contain' }} />
            )}
          ...
          <div className="contact-info text-xs text-gray-500">
            <div>{hospitalSettings.phone} | {hospitalSettings.email}</div>
        ...
        <div className="patient-info mb-4">
          ...
          <div className="info-row flex justify-between text-sm">
            ...
            <span className="value">{appointment.patient.firstName} {appointment.patient.lastName}</span>
          </div>
          <div className="info-row flex justify-between text-sm">
          ...
          <div className="info-row flex justify-between text-sm">
            ...
            <span className={`priority-badge priority-${appointment.priority.toLowerCase()}`}>
        ...
        <div className="appointment-info mb-4">
          <h3 className="font-bold text-gray-800 mb-2">Appointment Details</h3>
          ...
          <div className="info-row flex justify-between text-sm">
            ...
            <span className="value">{formatDate(appointment.session.date || appointment.createdAt)}</span>
          </div>
          <div className="info-row flex justify-between text-sm">
            <span className="label font-semibold">Time:</span>
            <span className="value">{formatTime(appointment.session.startTime)} - {formatTime(appointment.session.endTime)}</span>
          </div>
        </div>
        ...
        <div className="footer text-center text-xs text-gray-500 mt-4 pt-4 border-t border-gray-200">
          ...
          <div className="mt-1 font-semibold">Keep this token safe for your appointment</div>
        </div>
      </div>
      </div>
    </div>
