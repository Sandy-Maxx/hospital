...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("GET /api/imaging/requests failed", e);
    return NextResponse.json({ error: "Failed to fetch Imaging requests" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { admissionId, patientId, doctorId, procedureId, customName, priority, notes, requestedBasePrice, contrastReq } = body;
    ...
    const rec = await prisma.imagingRequest.create({
      data: {
        admissionId,
        patientId,
        doctorId,
    ...
    console.error("POST /api/imaging/requests failed", e);
    return NextResponse.json({ error: "Failed to create Imaging request" }, { status: 500 });
  }
}

// PUT /api/imaging/requests
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const upd = await prisma.imagingRequest.update({ where: { id }, data });
    return NextResponse.json({ request: upd });
  } catch (e) {
    console.error("PUT /api/imaging/requests failed", e);
    return NextResponse.json({ error: "Failed to update Imaging request" }, { status: 500 });
  }
}
