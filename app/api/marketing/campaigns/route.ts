...
async function selectAudience(audience: any) {
  ...
  if (selector === "VISITED_LAST_PERIOD" && havePeriod && since) {
    ...
    selector === "BOOKED_NOT_VISITED_LAST_PERIOD" &&
    havePeriod &&
    since
  ) {
    ...
    const rawConditions: string[] = Array.isArray(audience?.conditions)
      ? audience.conditions
      : [];
    ...
    if (terms.length > 0) {
      ...
      const pres = await prisma.prescription.findMany({
        where: {
          ...
          OR: [
            ...
            ...terms.map((t) => ({
              symptoms: { contains: t, mode: "insensitive" as const },
            })),
            ...terms.map((t) => ({
      ...
      const cons = await prisma.consultation.findMany({
        where: {
          ...
          OR: [
            ...
            ...terms.map((t) => ({
              chiefComplaint: { contains: t, mode: "insensitive" as const },
            })),
          ],
        },
        select: { patientId: true },
      ...
      const ids = new Set<string>([
        ...pres.map((p) => p.patientId),
        ...cons.map((c) => c.patientId),
      ]);
  ...
  selected = selected.filter((p) => {
    if (audience?.gender && p.gender && p.gender !== audience.gender)
      return false;
...
async function deliverCampaign(c: Campaign) {
  ...
  for (const p of targets) {
    ...
    try {
      ...
      if (c.channels?.email && email)
        await sendEmail(email, c.name || "Campaign", html);
