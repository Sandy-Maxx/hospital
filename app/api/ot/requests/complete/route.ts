...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const reqRec = await prisma.otRequest.findUnique({ where: { id } });
    ...
    if (reqRec.procedureId) {
      const proc = await prisma.oTProcedure.findUnique({ where: { id: reqRec.procedureId } });
    ...
    const ledger = await postLedgerBase(reqRec.admissionId, reqRec.patientId, base, `OT Base - ${name}`, `OT:${code}:BASE`);
    if (!ledger.ok) {
      const err = await ledger.json().catch(()=>({}));
      return NextResponse.json({ error: err.error || 'Ledger post failed' }, { status: 500 });
    }

    const upd = await prisma.otRequest.update({
      where: { id },
    ...
    console.error('POST /api/ot/requests/complete failed', e);
