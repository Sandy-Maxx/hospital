...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ dispatchedTests, allDispatched: dispatchedTests.length > 0 })
}
...
export async function POST(request: NextRequest) {
  ...
  const { prescriptionId, tests } = body as { prescriptionId?: string; tests?: string[] }
  if (!prescriptionId || !Array.isArray(tests)) return NextResponse.json({ error: 'Invalid payload' }, { status: 400 })
  const states = loadStates()
