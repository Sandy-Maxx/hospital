...
export async function POST(request: NextRequest) {
  ...
  const { prescriptionId, link, send } = body as {
    prescriptionId?: string;
    link?: { url: string; passcode: string; expiresAt?: string };
    send?: { channel: string; to?: string; at?: string };
  };
  if (!prescriptionId)
    return NextResponse.json(
      { error: "Missing prescriptionId" },
  ...
  if (send)
    next.sends = [
      ...(current.sends || []),
      { ...send, at: send.at || new Date().toISOString() },
    ];
  states[key] = next;
  saveStates(states);
