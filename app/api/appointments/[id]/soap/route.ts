...
export async function PATCH(
  ...
  try {
    ...
    if (appt.notes) {
      try {
        existing = JSON.parse(appt.notes);
      } catch {
        existing = { text: appt.notes };
      }
    }
    ...
    let editedBy = Array.isArray((existing as any)?.editedBy)
      ? ((existing as any).editedBy as any[])
      : [];
    ...
    if (!filledBy) {
      ...
      const last =
        editedBy.length > 0 ? editedBy[editedBy.length - 1] : filledBy;
      if (
        last?.id !== currentUserMeta.id ||
        last?.role !== currentUserMeta.role
      ) {
    ...
    const merged = {
      ...
      soapNotes: {
        subjective:
          soapNotes?.subjective || existing?.soapNotes?.subjective || "",
        objective: soapNotes?.objective || existing?.soapNotes?.objective || "",
        assessment:
          soapNotes?.assessment || existing?.soapNotes?.assessment || "",
