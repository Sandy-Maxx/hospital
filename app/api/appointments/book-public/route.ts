...
export async function POST(request: NextRequest) {
  try {
    ...
    const settings = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/settings/hospital`)
    ...
    const unavailabilityRules = await prisma.doctorAvailability.findMany({
      where: {
        ...
        OR: [
          { endDate: null },
          { endDate: { gte: new Date(sessionDateStr) } }
        ]
      }
    })
    ...
    if (!patient) {
      ...
      patient = await prisma.patient.update({
        ...
        data: {
          ...
          dateOfBirth: validatedData.dateOfBirth ? new Date(validatedData.dateOfBirth) : patient.dateOfBirth,
