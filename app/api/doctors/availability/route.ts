...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      session.user.role === "DOCTOR" &&
      validatedData.doctorId !== session.user.id
    ) {
      return NextResponse.json(
  ...
  try {
    ...
    if (!session || !["ADMIN", "DOCTOR"].includes(session.user.role)) {
      return NextResponse.json(
        {
          error:
            "Unauthorized. Only admins and doctors can delete availability rules.",
        },
        { status: 401 },
      );
    }
    ...
    if (
      session.user.role === "DOCTOR" &&
      availabilityRule.doctorId !== session.user.id
    ) {
      return NextResponse.json(
        { error: "Doctors can only delete their own availability rules" },
        { status: 403 },
      );
    }
