...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!sessionId && !date) {
      return NextResponse.json({ error: 'Session ID or date is required' }, { status: 400 })
    }
    ...
    if (sessionId) {
      ...
      if (sessionAssignments.length === 0) {
        ...
        if (sessionDate) {
          const availableDoctors = await getAvailableDoctorsForDate(allDoctors, sessionDate.date)
          return NextResponse.json({ doctors: availableDoctors })
        }
      }

      // Filter assigned doctors by availability
      const assignedDoctors = sessionAssignments.map(assignment => assignment.doctor)
      ...
      if (session) {
        const availableDoctors = await getAvailableDoctorsForDate(assignedDoctors, session.date)
        return NextResponse.json({ doctors: availableDoctors })
      }
    }
    ...
    if (date) {
      ...
      const availableDoctors = await getAvailableDoctorsForDate(allDoctors, targetDate)
      return NextResponse.json({ doctors: availableDoctors })
    }
...
async function checkDoctorAvailability(doctorId: string, date: Date): Promise<boolean> {
