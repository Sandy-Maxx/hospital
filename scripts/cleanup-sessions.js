...
async function cleanupSessions() {
  try {
    ...
    const settingsPath = path.join(__dirname, '..', 'data', 'hospital-settings.json')
    const settingsData = JSON.parse(fs.readFileSync(settingsPath, 'utf8'))
    const activeTemplates = settingsData.sessionTemplates.filter(template => template.isActive)
    ...
    const sessionsToDelete = existingSessions.filter(session => 
      !validShortCodes.includes(session.shortCode)
    )
